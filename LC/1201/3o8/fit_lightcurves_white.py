    ...
    if config['res'] is not None:
        if config['res'] == 'pixel':
            ...
            if config['npix']<len(flux[0]):
                ...
                nbin = int(nwave / npix)
                flux = np.nansum(np.reshape(flux, (nint, nbin, npix)), axis=2)
                err = np.sqrt(np.nansum(np.reshape(err, (nint, nbin, npix))**2,axis=2))
                # Calculate mean wavelength per bin.
                wave_bin = np.nanmean(np.reshape(wave, (nint, nbin, npix)), axis=2)
                wave_low = np.nanmax(np.reshape(wave, (nint, nbin, npix)), axis=2)
                wave_up = np.nanmin(np.reshape(wave, (nint, nbin, npix)), axis=2)
                wave, wave_low, wave_up = wave_bin[0], wave_low[0], wave_up[0]
